...
import de.telekom.pni.rmstest.backend.global.GlobalVarWetterInfo_Preview;
import de.telekom.pni.rmstest.backend.manager.PropertiesManager_New;
import de.telekom.pni.rmstest.backend.manager.WebDriverManager_New;
...
public abstract class GenericTest_New implements RMSTest_New {
    ...
    public final Properties testRun = PropertiesManager_New.getInstance().getProperties("testrun");
    ...
    public void setFailed() {
        logFailureCheckpoint("Testrun failed", "Set failed method",
    ...
    /**
     * Get the step logger for this test run.
     *
     * @return Step logger (existing or new).
     */
    ...
    public Report getReport() {
        if (this.report == null) {
            this.report = new Report();
            this.report.setStatus(false); // default status for fresh reports
            this.report.setTestName(this.getClass().getName());
        }
        return report;
    }
    ...
    public WebDriver getBrowser() {
        ...
        if (this.runningConfiguration == null) {
            this.runningConfiguration = RunningConfiguration_New.defaultFFOnAB();
            ...
            getStepLogger()
                    ...
                    .logStep(
                            new Step(
                                    ...
                                    "MUST BE DISABLED FOR REGULAR RUNS."));
        }
        try {
//			this.browser = utils.getBrowser(getRunningConfiguration());
            this.browser = driverManager.setUpDriver(runningConfiguration);
            getStepLogger().logStep( // pass opening browser
                    new Step(true, "Open browser", "Browser is opened",
                            "Browser was opened successfully.",
                            "Open the browser"));

        } catch (Exception ex) {
            getStepLogger().logStep( // error opening browser
                    new Step(false, "Open browser", "Browser is opened",
                            "Error occured while opening browser.", ex
                            .getStackTrace().toString()));
            return null;
        }
        return this.browser;
    ...
    public WebDriver setBrowserFirefox(FirefoxProfile profile) {
        ...
        if (this.browser != null)
            return this.browser;
        if (this.runningConfiguration == null) {
            this.runningConfiguration = RunningConfiguration_New
                    .defaultFFOnSP();
            getStepLogger()
                    // log error
                    .logStep(
    ...
    public String getMachine() {
        try {
            ...
            getStepLogger().logStep(
                    new Step(false, "Trying to get machine",
                            "Machine to test on (as string)", "Error occured.",
    ...
    public void navigate(String url) {
        ...
        //Wrid nicht mehr benötig, da die Url dirket von den Klassen geliefert werden.
        //Püft, ob das Testing-Suite ausgeführt wird.
	
		/*if (runningConfiguration!= null && url.contains("www.wetter.info"))
		{
			String patter = "www.wetter.info";
			String machineToRun = runningConfiguration.getMachine();
			
			String testWetter = testRun.getProperty("run.Test-Wetter");
			String wirkWetter = testRun.getProperty("run.Wirktest-Wetter");
			String abnahmeWetter = testRun.getProperty("run.Abnahmetest-Wetter");
			String testTOnline = testRun.getProperty("run.Test-T-Online");
			String wirkTOnline = testRun.getProperty("run.Wirktest-T-Online");
			String abnahmeTOnline = testRun.getProperty("run.Abnahmetest-T-Online");
			
			if (machineToRun.contains(testWetter) ||
				machineToRun.contains(wirkWetter) ||
				machineToRun.contains(abnahmeWetter) ||
				machineToRun.contains(testTOnline) ||
				machineToRun.contains(wirkTOnline) ||
				machineToRun.contains(abnahmeTOnline)) 
			{
				String preURL = url.replaceAll(patter,"");
				url = machineToRun + preURL;				
			}
		}*/
        ...
        try {
            ...
            if (this.getRunningConfiguration().getBrowser().equals("IE")) {
                //Hat zu probleme gefürht daher auskommenkiert. Ist aber wichtig, falls eien https seite aufgerufen muss....
                //getBrowser().get("javascript:document.getElementById('overridelink').click();");
                //wd. getWindowHandle()

                //habe ich gemach um den Entwicklermodus des IE zu starten!!
                //Actions action  = new Actions(wd);
                //Entwicklermudus an
                //action.keyDown(Keys.F12);
                //action.perform();
                //Enwicklermudus aus
                //action.keyUp(Keys.F12);
                //action.perform();
            }

            getBrowser().manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
            getStepLogger().logStep( // pass trying to navigate
                    new Step(true, "Trying to navigate",
                            "Target site should be opened.",
                            "Target location was opened successfully.",
                            "Open the Target location : " + url));
            try {
                Thread.sleep(2000);
            ...
            getStepLogger().logStep(
                    new Step(false, "Trying to navigate",
                            "Target site should be opened.",
                            "Error occured while navigating to " + url, ex
                            .getStackTrace().toString()));
        }
    }
    ...
    public WebElement getByXPath(String xPath) {
        try {
            ...
            getStepLogger().logStep(
                    new Step(false, "Getting element by xPath",
                            "Element should be identified.",
                            "Error occured while getting element for " + xPath,
                            ex.getMessage()));
            return null;
        }
    }
    ...
    public void closeBrowser() {
        ...
        try {
            if (this.browser != null) {
                ...
                browser.quit();
                this.browser = null;
                logSuccessCheckpoint("Closing browser",
                        "Browser successfully closed.");
            }
        } catch (Exception ex) {
            getStepLogger().logStep(
                    new Step(false, "Closing browser",
                            "Browser should be closed.",
                            "Error occured while closing browser.", ex
                            .getStackTrace().toString()));
        }
    }

    /**
     * * Aborts the running 	 closes the Browser, finalizes the report,
    ...
    public Report abortTest(String cause) {
        ...
        logFailureCheckpoint("Test wird abgebrochen", "Ursache: " + cause, "");
    ...
    public Report finalizeReport(String testName) {
        report.setSteps(getStepLogger().getSteps());
            ...
            getStepLogger()
                    .logStep(
                            new Step(
                                    ...
                                    "Supplied xPath doesn't identify a link. Identified element's tag is: "
                                            + link.getTagName().toLowerCase(),
                                    "Supplied xPath doesn't identify a link. Element tag is: "
                                            + link.getTagName().toLowerCase()));
            return false;
        }
        // click the link
        getStepLogger()
                .logStep(
                        new Step(
                                true,
                                "Click link method",
                                "Supplied xPath identifies a link (html tag \"a\")",
     ...
     * to report steps.
     *
     * @param linkXPath XPath identifying the link
    ...
    public WebDriver clickLinkAndSwitchToPopup(String linkXPath) {
        ...
        List<String> windows = new ArrayList<String>(getBrowser()
                .getWindowHandles());
        ...
        List<String> windowsNew = new ArrayList<String>(getBrowser()
                .getWindowHandles());
    ...
    public boolean checkEqual(WebElement webel, String expectedText, String chekPointName) {
        String weText = null;
            ...
            getStepLogger()
                    .logStep(
                            new Step(
                                    ...
                                    expectedText,
                                    "Error occured while getting webelement's text property: ",
                                    ex.getStackTrace().toString()));
            return false;
        }
        ...
        if (weText != null && expectedText != null && expectedText.length() > 0
            ...
            logSuccessCheckpoint(chekPointName, expectedText);
            return true;
        } else {
            logFailureCheckpoint(chekPointName, expectedText, weText);
            return false;
        }
    }
    ...
    public boolean checkCSS(WebElement webel, String CSSType, String expectedCSS, String checkPointName) {
        ...
        try {
            ...
            if (actualCSS.equals(expectedCSS)) {
                logSuccessCheckpoint(checkPointName, "Actual CSS value (" + actualCSS + ") equals the expected value (" + expectedCSS + ")");
                return true;
            } else {
                logFailureCheckpoint(checkPointName, "Actual CSS value equals the expected value ", "Actual CSS value (" + actualCSS + ") does not equal the expected value (" + expectedCSS + ")");
                return false;
            }
        } catch (Exception ex) {
            getStepLogger()
    ...
    public boolean checkEqualStrings(String expectedString, String actualString, String checkPointName) {
        ...
        if (expectedString.equalsIgnoreCase(actualString)) {
            ...
            logFailureCheckpoint(checkPointName, "The strings are equal.",
                    "Specified strings are not equal. Expected: "
    ...
    public boolean checkIsPartOfString(String subjectString, String subString,
        ...
        if (subjectString.contains(subString)) {
            logSuccessCheckpoint(checkPointName, "The subject string "
                    + subjectString + " contains the substring " + subString + ".");
            return true;
        } else {
            logFailureCheckpoint(checkPointName, "The subject string "
                            + subjectString + " should contain the substring " + subString + ".",
                    "The subject string "
                            + subjectString + " DOES NOT contain the substring " + subString + ".");
            return false;
        }
    ...
    public boolean isScaled(WebElement webel, Dimension DefaultDimension,
            ...
            getStepLogger()
                    .logStep(
                            new Step(
                                    ...
                                    "Error occured while getting webelement's Size property: ",
                                    ex.getStackTrace().toString()));
            return false;
        }
        // we have the strings; compare
        if (weDimension != null
            ...
            logSuccessCheckpoint(chekPointName,
                    "Bild ist skaliert vom " + DefaultDimension.toString()
                            + " auf " + weDimension.toString());
            return true;
        } else {
            logFailureCheckpoint(chekPointName,
                    "Bild wird skaliert Dimension ungleich: "
                            + DefaultDimension.toString(),
                    "Bild wurde nicht skaliert, Actual Dimension: "
                            + weDimension.toString());
            return false;
        }
    }
    ...
    public boolean imgIsNOTDisplayed(WebElement img, String checkPointName) {
        ...
        try {
            ...
            if (runningConfiguration.getBrowser().equals("FF")) {
                // Für FF
                if (!img.isDisplayed()) {
                    logSuccessCheckpoint(checkPointName,
            ...
            // Für FF
            if (!img.isDisplayed()) {
                logSuccessCheckpoint(checkPointName, "Image does not exists.");
                return true;
            } else {
                logFailureCheckpoint(checkPointName, "Image does not exists.",
                        "Image is exist");
                return false;
            }
    ...
    public boolean imgIsDisplayed(WebElement img, String checkPointName) {
        if (img == null) {
            logFailureCheckpoint(checkPointName, "Image exists.",
                    "Image does not exists (provided WebElement is null)");
            return false;
        }
        if (img.isDisplayed()) {
            logSuccessCheckpoint(checkPointName, "Image exists.");
            return true;
        } else {
            logFailureCheckpoint(checkPointName, "Image exists.",
    ...
    public boolean checkExists(WebElement element, String checkPointName) {
        ...
        if (element != null) {
            logSuccessCheckpoint(checkPointName, "Webelement exists.");
            return true;
        } else {
            logFailureCheckpoint(checkPointName, "Webelement exists.",
    ...
    public boolean checkMatchesInList(List<String> theList, String toSearch,
        ...
        if (!success) {
            logFailureCheckpoint(checkPointName, "Searched for " + toSearch
                    + " in the specified list and expected it "
                    + shouldMaybeExist + " in the list.", "Check failed.");
            return false;
        } else {
            logSuccessCheckpoint(checkPointName, "Searched for " + toSearch
                    + " in the specified list and expected it "
                    + shouldMaybeExist + " in the list.");
            return true;
        }
    }
    ...
    public boolean checkIsTrue(boolean subject, String subjectName,
                               String checkPointName) {
        if (subject) {
            logSuccessCheckpoint(checkPointName, subjectName
                    + " has boolean value TRUE.");
            return true;
        } else {
            logFailureCheckpoint(checkPointName, subjectName
                    + " must have boolean value TRUE.", subjectName
    ...
    public void Logoutput_element_is_not_displayed(WebElement element,
        ...
        try {
            if (element.isDisplayed() == false) {
                logSuccessCheckpoint("Element is not displayed", expected);

            } else {
                logFailureCheckpoint("Element is displayed", expected, actual_failure);
            }
        } catch (Exception e) {
            logFailureCheckpoint("Is a element visible ?", "Element should NOT be identified.", "Error occured: "
    ...
    public void Logoutput_element_is_displayed(WebElement element, String expected, String actual_failure) {
        try {
            if (element.isDisplayed() == true) {
                logSuccessCheckpoint("Element is displayed", expected);

            } else {
                logFailureCheckpoint("Element is not displayed", expected,
                        actual_failure);
            }
        } catch (Exception e) {
            logFailureCheckpoint("Is a element visible ?", "Element should NOT be identified.", "Error occured: "
    ...
    public boolean checkpoint_two_ElementsInSequence(WebElement element1,
        ...
        try {
            ...
            System.out.println("Konnte die Bildpositionen nicht finden, "
                    + "da die Bilder nicht schnell geladen wurden "
                    + e.getMessage());
    ...
    public boolean checkWithinSourceCode(String expectedText,
        ...
        if (lio > 0) {
            ...
            logFailureCheckpoint(checkPointName, expectedText, "String \""
                    + expectedText
                    + "\" could not be found in the Source Code of the page.");
            return false;
        }
    ...
    public boolean checkDoesNOTExist(String xPath, String checkPointName) {
        ...
        try {
            ...
            logFailureCheckpoint(checkPointName,
                    "Element should NOT be identified.", "Error occured: "
                            + ex.getStackTrace().toString());
            return false;
        }
        //
    ...
    public boolean checkRegExp(String patternString, String stringToSearchIn,
        ...
        try {
            if (Pattern.matches(patternString, stringToSearchIn)) {
                if (positiveSearch) {
                    // searched and found
                    logSuccessCheckpoint(checkPointName, patternString
                            + " matched " + stringToSearchIn);
                    return true;
                } else {
                    // unexpectedly found
                    logFailureCheckpoint(checkPointName, "Pattern "
                            + patternString + " should not match "
                            + stringToSearchIn + ".", "Pattern "
                            + patternString + " unexpectedly matches "
                            + stringToSearchIn + ".");
                    return false;
                }
            } else { // no match
                if (positiveSearch) {
                    ...
                    logFailureCheckpoint(checkPointName, "Pattern "
                            + patternString + " should match "
                            + stringToSearchIn + ".", "Pattern "
                            + patternString + " doesn't match "
                            + stringToSearchIn + ".");
                    return false;
                } else {
                    // not found as expected
                    logSuccessCheckpoint(checkPointName, patternString
                            + " doesn't match " + stringToSearchIn);
                    return true;
                }
            }
        } catch (Exception ex) {
            logFailureCheckpoint(checkPointName,
                    "Trying to compile RegExp Pattern.", "Error occured: "
                            + ex.getStackTrace().toString());
            return false;
        }
    }
    ...
    public boolean checkEqual(int actualValue, int expectedValue, String checkPointName) {

        if (expectedValue == actualValue) {
            logSuccessCheckpoint(checkPointName,
                    "Compared integer values are equal. Both values are "
                            + actualValue);
            return true;
        } else {
            logFailureCheckpoint(checkPointName,
                    "Compared integer values are equal.",
                    "Compared integer values are NOT equal. Expected: "
                            + expectedValue + "; Actual Value " + actualValue);
            return false;
        }
    }

    public boolean checkIsClickable(String xPath, boolean positive, String checkPointName) {
            ...
            try {
                if (parent.getTagName().equalsIgnoreCase(TAG_LINK)) {
                    ...
                    if (positive) { // searched for a link and found a link
                        logSuccessCheckpoint(checkPointName,
                                "WebElement denoted by " + xPath
                                        + " is clickable as expected.");
                        return true;
                    } else {
                        logFailureCheckpoint(checkPointName,
                                "WebElement denoted by " + xPath
        ...
        // it's NOT a link
        if (!positive) {
            logSuccessCheckpoint(checkPointName, "WebElement denoted by "
                    + xPath + " is NOT clickable as expected.");
            return true;
        } else {
            logFailureCheckpoint(checkPointName, "WebElement denoted by "
                    + xPath + " is clickable.", "WebElement is NOT clickable.");
            return false;
        }
    }
    ...
    public boolean isInputfieldEditable(String xpathToInputfield, String checkPointName) {
        ...
        try {
            ...
            if (element != null && element.getTagName().equalsIgnoreCase("input")) {
                ...
                logFailureCheckpoint(checkPointName, "", "This element is NULL or NOT an Input Field");
                return false;
            }
        } catch (NoSuchElementException e) {

            logFailureCheckpoint(checkPointName, "Element should NOT be identified.", "Error occured: " + e.getStackTrace().toString());
            return false;

        } catch (Exception ex) {
            logFailureCheckpoint(checkPointName, "An unexpected error has occurred: ", "Error occured: " + ex.getStackTrace().toString());
            return false;
        }
    ...
    public void logSuccessCheckpoint(String checkPointName, String expected) {
        getStepLogger().logStep(
                new Step(true, "Checkpoint " + checkPointName, expected,
                        expected, "Checkpoint passed."));
    }

    public void logFailureCheckpoint(String checkPointName, String expected, String actual) {
        getStepLogger().logStep(
                new Step(false, "Checkpoint " + checkPointName, expected,
                        actual, "Checkpoint failed."));
    }
    ...
    public boolean metadataexists(String metadata, String Attribute,
        ...
        if (Meta.size() > 0) {
            ...
            if (Result) {
                logSuccessCheckpoint("check if metadata \"" + metadata
                                + "\" with the  Atribute \"" + Attribute
                                + "\" equals \"" + Attributevalue + "\"",
                        Attributevalue);
            } else {
                logFailureCheckpoint("check if metadata \"" + metadata
                        + "\" with the  Atribute \"" + Attribute
                        + "\" equals \"" + Attributevalue + "\"", "\""
    ...
    public boolean checkValueOfAttribute(WebElement webel, String attribute, String expected, String checkPointName) {
        try {
            if (webel.getAttribute(attribute).equals(expected)) {
                logSuccessCheckpoint(checkPointName,
                        "The value of the attribute " + attribute + " of the element matches the expected value "
                                + expected);
                return true;
            } else {
                logFailureCheckpoint(checkPointName,
            ...
            logFailureCheckpoint(checkPointName, "Error occured while getting webelement's attribute",
                    e.getStackTrace().toString());
            return false;
        }
    }
    ...
    public boolean checkCountElements(String xPath, int expectedCount, String checkPointName) {
        ...
        if (actualCount == expectedCount) {
            logSuccessCheckpoint(checkPointName, "The number of Newsitems displayed is " + expectedCount);
            return true;
        } else {
            logFailureCheckpoint(checkPointName,
    ...
    public boolean clickOnElement(String xPath, String checkPointName) {
        try {
            ...
            if (element != null) {
                element.click();
                logSuccessCheckpoint(checkPointName, "Element has been clicked");
                return true;
            } else {
                logFailureCheckpoint(checkPointName, "Element has been clicked",
    ...
    public void selectFilter(String filter, String element, String checkpont) {
        ...
        try {
            ...
            logSuccessCheckpoint(checkpont, "Filter was changed");

        } catch (NoSuchElementException e) {
            logFailureCheckpoint(checkpont, "", e.getMessage());
        }
        pauseTest(2000);
    }
 ...
 */
	/*private boolean elementIsDisplay(String xPath) {
		
		WebElement autoSuggest = getBrowser().findElement(By.xpath(xPath));
		if (autoSuggest.getSize().height >2){
			//System.out.println(autoSuggest.getCssValue("display"));
			return true;
		}
		return false;
	}*/
    ...
    public void setInputFeldValue(String xPath, String value) {
        WebElement element = getByXPath(xPath);
        element.sendKeys(value);
    }

    public void setInputFeldValueClick(String xPath, String value, String clickValue, GlobalVar gVar) {
        WebElement element = getByXPath(xPath);
        element.sendKeys(value);
        this.pauseTest(1000);
        List<WebElement> listOfAutoSuggest = getListAutoSuggest(gVar._AUTOSUGGEST, gVar._AUTOSUGGEST_LIST);

        for (WebElement webElement : listOfAutoSuggest) {

            if (webElement.getText().contains(clickValue)) {
                webElement.click();
                return;
            }
        }
    ...
    public List<WebElement> getListSuchergebnis(String suchergebnisList) {

        if (getWebElement(suchergebnisList).isDisplayed()) {
            List<WebElement> e = getBrowser().findElements(By.xpath(suchergebnisList));
            return e;
        }
        return null;
    }

    public List<WebElement> getListALink(String alinks) {
        try {
            List<WebElement> e = getBrowser().findElements(By.xpath(alinks));
            return e;
        } catch (Exception f) {
    ...
    public void openSuchergebnisOrte(String xPath) {
        List<WebElement> temp = getListSuchergebnis(xPath);
        int counter = temp.size() + 1;

        for (int i = 2; i < counter; i++) {
            getWebElement(".//*[@id='Twieser']/h4[" + i + "]/span").click();
            this.pauseTest(250);
        }
    ...
    public void after() {
        try {
            ...
            printReport(getReport());
        } catch (Exception e) {
            log.info("Nothing to close oder report");
        }
    }
    ...
    public GlobalVar getGlobalVar() {
        ...
        if (runningConfiguration != null) {
            ...
            if (machineToRun.contains(wirkWetter)) {
                temp = new GlobalVarWetterInfo();
            } else if (machineToRun.contains(testWetter) || machineToRun.contains(abnahmeWetter)) {
                temp = new GlobalVarWetterInfo_Preview();
            } else if (machineToRun.contains(testTOnline) || machineToRun.contains(wirkTOnline) || machineToRun.contains(abnahmeTOnline)) {
                temp = new GlobalVarTOnline();
            } else if (machineToRun.contains(testTOnline) || machineToRun.contains(wirkTOnline) || machineToRun.contains(abnahmeTOnline)) {
                temp = new GlobalVarTOnline_Preview();
            }
        }
        return temp;
    }
    ...
    public void openNewTab() {
        getBrowser().findElement(By.tagName("body")).sendKeys(Keys.CONTROL + "t");
        // ArrayList<String> tabs = new ArrayList<String> (getBrowser().getWindowHandles());
        //getBrowser().switchTo().window(tabs.get(0));

    }

    public void closeTab(WebDriver webDriver, int tabNumber) {
        webDriver.quit();
    }
    ...
    public WebDriver openNewWindow() {
        WebDriver temp;
        try {
            temp = driverManager.setUpDriver(runningConfiguration);
            getStepLogger().logStep( // pass opening browser
                    new Step(true, "Open browser", "Browser is opened",
                            "Browser was opened successfully.",
                            "Open the browser"));
    ...
    public void closeWindow(WebDriver webDriver) {
        webDriver.close();
    }

    public List<String> getWindowsHandles() {

        Set<String> winSet = getBrowser().getWindowHandles();
